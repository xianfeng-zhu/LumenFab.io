...
本页把 CPO 光源系统作为一条完整链路来理解：从 InP / III-V 材料为什么能够发光开始，经过衬底、外延、有源区、波导、腔体和 DFB 光栅，再回到激光器芯片如何进入 <TermNote label="TOSA" note="Transmitter Optical Sub-Assembly，发射端光学子组件，通常把激光器、耦合光学、监控探测器和封装接口组织在一起。" />、<TermNote label="ELSFP" note="External Laser Small Form-Factor Pluggable，OIF 定义的外置光源可插拔形态，用来给光引擎提供连续光。" /> 或 optical engine 输入端。

这里的“光源系统”不是单颗激光器芯片的同义词。它同时包括材料和器件物理，也包括光源放在封装内还是封装外、如何接入光引擎、如何测试以及如何维护。本文默认语境是网络交换 CPO：光引擎靠近数据中心交换芯片（<TermNote label="switch ASIC" note="数据中心交换机中的网络交换芯片，不是 GPU / TPU die；本页默认指网络交换 CPO 语境。CPO 把光引擎放到它附近，以缩短 switch ASIC 到光引擎之间的高速电信号路径。" />）。GPU / TPU / XPU 封装中的 optical I/O 属于相邻路线，后续需要单独比较。
...
因此，CPO 光源不是单纯的器件选择问题，而是一条从材料、器件、封装到光引擎输入的工程链路。
...
```mermaid
graph LR
    RM["In / P 原料"] --> SUB["InP 衬底"]
    SUB --> EPI["外延结构"]
    EPI --> ACTIVE["QW / MQW 有源区"]
    ACTIVE --> CAV["波导 / 腔体 / DFB 光栅"]
    CAV --> CHIP["DFB / CW laser chip"]
    CHIP --> PKG["TOSA / ELSFP / 光引擎内光源"]
    PKG --> OE["optical engine 输入光"]
    OE --> PIC["PIC / 调制器"]
```

后续各节沿这条路径展开。当前版本先建立结构和关键概念，细节会逐步补齐。

## 材料层：为什么是 InP / III-V

通信光源通常依赖 III-V 族半导体材料体系。原因不只是“某种材料可以做激光器”，而是它需要同时满足发光效率、波长范围、外延兼容性和器件可靠性。
...
激光器的微观结构不是在一块均匀材料中刻出来的，而是在衬底上通过外延形成多层半导体结构。后续会继续补充每一步的工艺窗口和缺陷控制。
...
半导体激光器通常是电泵浦器件。外部电流并不是直接变成光，而是先注入电子和空穴；电子和空穴在有源区复合，释放出的能量形成光子。
...
<TermNote label="DFB" note="Distributed Feedback，分布式反馈激光器或反馈结构，利用沿腔体分布的光栅选择稳定单模波长。" /> 不是材料名称，也不是有源区名称，而是激光器腔体 / 反馈结构。它通过沿传播方向分布的光栅提供反馈，使满足 Bragg 条件的波长获得更强选择性。
