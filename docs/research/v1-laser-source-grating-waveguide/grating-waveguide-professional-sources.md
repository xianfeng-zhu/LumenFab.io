# 波导、DFB 光栅与工艺容差：专业资料记录

日期：2026-05-21

适用范围：为 `src/pages/components/laser-source.mdx` 第 3 节“波导、DFB 光栅与工艺容差”准备资料。该章节应解释 InP / DFB 激光器如何从外延层结构进入器件加工：先形成能够限制光场的波导，再通过 DFB 光栅建立沿传播方向的波长选择和反馈，并说明这些纳米级结构如何形成量产壁垒。
...
10. 波导与 DFB 工艺的量产壁垒来自跨工序闭环：光刻和刻蚀决定脊宽、刻蚀深度、侧壁粗糙度和光栅周期；再生长和清洗决定 BH 或埋入式光栅界面质量；测试和老化数据需要反向关联到 `n_eff`、`kappa`、损耗、阈值、SMSR 和中心波长的一致性。成熟能力体现在材料、图形、刻蚀、再生长和筛选规则之间的稳定工艺窗口，而不只是一张截面图。
