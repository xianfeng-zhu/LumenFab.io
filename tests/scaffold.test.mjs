...
test("light source component page is organized as a layer map", async () => {
  ...
  assert.match(page, /## 本页回答的问题/);
  assert.match(page, /## CPO 为什么会放大光源问题/);
  assert.match(page, /## 光源位置与系统边界/);
  assert.match(page, /### 内置与外置方案示意/);
  assert.match(page, /### 标准语境：ELS 和 ELSFP/);
  assert.match(page, /## 概念层级边界/);
  assert.match(page, /## 后续学习路径/);
  assert.match(page, /## 关键评估指标/);
  ...
  assert.match(page, /DFB 是激光器腔体 \/ 反馈结构/);
  assert.match(page, /光学输出/);
  assert.match(page, /噪声与单模性/);
  assert.match(page, /效率与热稳定/);
